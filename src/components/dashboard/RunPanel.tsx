...
export default function RunPanel() {
  ...
  return (
    <>
      {!open && <button type="button" onClick={() => setOpen(true)} className="fixed right-0 top-auto bottom-20 z-30 flex translate-y-0 items-center gap-1 rounded-xl border border-[#1e4361] bg-[#0b1b30] px-2 py-2 text-[10px] font-bold uppercase tracking-wider text-primary shadow-xl [writing-mode:horizontal-tb] sm:top-1/2 sm:bottom-auto sm:-translate-y-1/2 sm:rounded-l-xl sm:rounded-r-none sm:py-4 sm:[writing-mode:vertical-rl]">Run panel <ChevronRight className="h-3.5 w-3.5" /></button>}
