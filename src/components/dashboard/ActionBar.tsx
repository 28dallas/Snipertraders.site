...
export default function ActionBar() {
  ...
  return (
    <div className="fixed bottom-3 left-1/2 z-40 flex w-[calc(100%-1.5rem)] max-w-2xl -translate-x-1/2 items-center justify-between gap-2 rounded-2xl border border-[#1e4361] bg-[#09172a]/95 p-2 shadow-2xl backdrop-blur-xl">
      <button type="button" onClick={handleRun} className="flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-black text-black transition hover:brightness-110"><Play className="h-4 w-4 fill-current" />Run</button>
      ...
      <div className="flex items-center gap-1.5 text-xs font-mono text-slate-300"><Clock3 className="h-3.5 w-3.5 text-primary" />{now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</div>
      ...
      <button type="button" onClick={() => setDark((value) => !value)} className="rounded-lg p-2 text-slate-300 hover:bg-white/10" title="Toggle theme">{dark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}<span className="sr-only">Toggle theme</span></button>
